...
async def gen_thumb(videoid):
    try:
        ...
        if 1==1:
            ...
            image4.text((10, 10), "NOTTYY MUSIC", fill="black", font = font1, align ="left")
            image4.text((10, 10), "NOTTYY MUSIC", fill="black", font = font1, align ="right") 
        ...
        if 1==1:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"http://img.youtube.com/vi/{videoid}/maxresdefault.jpg") as resp:
                    if resp.status == 200:
                        f = await aiofiles.open(
                            f"cache/qthumb{videoid}.jpg", mode="wb"
                        )
                        await f.write(await resp.read())
                        await f.close()

            youtube = Image.open(f"cache/qthumb{videoid}.jpg")
            image1 = changeImageSize(1280, 720, youtube)
            image2 = image1.convert("RGBA")
            background = image2.filter(filter=ImageFilter.BoxBlur(30))
            enhancer = ImageEnhance.Brightness(background)
